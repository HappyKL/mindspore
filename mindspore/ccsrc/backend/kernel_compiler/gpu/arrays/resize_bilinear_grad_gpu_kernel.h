...
#ifndef MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_RESIZE_BILINEAR_GRAD_H_
#define MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_RESIZE_BILINEAR_GRAD_H_
...
class ResizeBilinearGradGpuKernel : public GpuKernel {
 public:
  ...
  bool Launch(const std::vector<AddressPtr> &inputs, const std::vector<AddressPtr> &workspace,
    ...
    CalResizeBilinearGrad(input_size, input, input_shape_[0], input_shape_[1], input_shape_[2], input_shape_[3],
  ...
  bool Init(const CNodePtr &kernel_node) override {
    ...
    if (shape_size_ != RESIZEBILINEAR_DIMENSION) {
      MS_LOG(ERROR) << "Input is " << shape_size_ << "-D, but ResizeBilinear supports only "
                    << RESIZEBILINEAR_DIMENSION << "-D inputs.";
      return false;
    }
    input_size_ = 1;
    ...
    for (size_t i = 0; i < shape_size_; i++) {
      ...
      MS_LOG(INFO) << "output_shape " << i << ":" << output_shape[i] << "end.";
      output_shape_.push_back(output_shape[i]);
    }
    MS_LOG(INFO) << "Output shape " << output_size_ << "end.";
    output_size_ *= sizeof(T);
     MS_LOG(INFO) << "Out size " << output_size_ << "end.";
...
#endif  // MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_RESIZE_BILINEAR_GRAD_H_
